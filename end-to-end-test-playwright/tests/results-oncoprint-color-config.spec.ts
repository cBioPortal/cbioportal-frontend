...
test.describe
    .serial('oncoprint results-view clinical track color config', () => {
    ...
    async function pickColor(n: number, hex: string) {
        await page.locator(COLOR_PICKER_ICON).nth(n).click();
    ...
    test('color modal reflects user-selected colors', async () => {
        ...
        const option = byTestHandle(page, 'add-chart-option-mutation-spectrum');
        ...
        await expect(
            page.locator(COLOR_PICKER_ICON).nth(0).locator('rect')
        ).toHaveAttribute('fill', '#990099');
        await expect(
            page.locator(COLOR_PICKER_ICON).nth(1).locator('rect')
        ).toHaveAttribute('fill', '#109618');
        await expect(
            page.locator(COLOR_PICKER_ICON).nth(2).locator('rect')
