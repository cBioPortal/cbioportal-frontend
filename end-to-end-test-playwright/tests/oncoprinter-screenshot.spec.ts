 ...
 * hide VUS, don't color by type) or re-submit with custom gene/sample
 * orderings.
 *
...
const CLINICAL = '.oncoprinterClinicalExampleData';
const HEATMAP = '.oncoprinterHeatmapExampleData';
const SUBMIT = '.oncoprinterSubmit';
const MODIFY = '.oncoprinterModifyInput';
const GENES_INPUT = '.oncoprinterGenesInput';
const SAMPLES_INPUT = '.oncoprinterSamplesInput';
...
async function submitOncoprinter(
    page: Page,
    options: { genetic?: boolean; clinical?: boolean; heatmap?: boolean } = {}
) {
    await page.goto('/oncoprinter');
    if (options.genetic ?? true) {
        await expect(page.locator(GENETIC)).toBeVisible();
    }
    if (options.clinical) {
    ...
    if (options.genetic ?? true) await page.locator(GENETIC).click();
    if (options.clinical) await page.locator(CLINICAL).click();
    if (options.heatmap) await page.locator(HEATMAP).click();
    await page.locator(SUBMIT).click();
    await waitForOncoprint(page);
}

async function openGenesInputViaModify(page: Page) {
    await expect(page.locator(MODIFY)).toBeVisible();
    await page.locator(MODIFY).click();
    await expect(page.locator(GENES_INPUT)).toBeVisible();
}

async function openSamplesInputViaModify(page: Page) {
    await expect(page.locator(MODIFY)).toBeVisible();
    await page.locator(MODIFY).click();
...
test.describe('oncoprinter screenshot tests', () => {
    test('genetic only example data', async ({ page }) => {
        ...
        await expectOncoprintScreenshot(page, 'oncoprinter-genetic-only.png');
    });

    test('clinical only example data', async ({ page }) => {
        await submitOncoprinter(page, { genetic: false, clinical: true });
        await expectOncoprintScreenshot(page, 'oncoprinter-clinical-only.png');
    });

    test('heatmap only example data', async ({ page }) => {
        await submitOncoprinter(page, { genetic: false, heatmap: true });
        await expectOncoprintScreenshot(page, 'oncoprinter-heatmap-only.png');
    });

    test('all example data', async ({ page }) => {
        await submitOncoprinter(page, {
            genetic: true,
            clinical: true,
            heatmap: true,
        });
        await expectOncoprintScreenshot(page, 'oncoprinter-all-example.png');
    });

    test('all example data, annotated by OncoKb', async ({ page }) => {
        await submitOncoprinter(page, {
            genetic: true,
            clinical: true,
            heatmap: true,
        ...
        await expect(page.locator('.oncoprint-legend-div')).toContainText(
            'Inframe Mutation (putative driver)'
        );
        await setOncoprintMutationsMenuOpen(page, false);
        await expectOncoprintScreenshot(page, 'oncoprinter-oncokb.png');
    });

    test('clinical example data (second variant)', async ({ page }) => {
        await submitOncoprinter(page, { genetic: false, clinical: true });
        await expectOncoprintScreenshot(
            page,
            'oncoprinter-clinical-only-2.png'
        );
    });

    test('set gene order including all genes', async ({ page }) => {
        await submitOncoprinter(page, {
            genetic: true,
            clinical: true,
            heatmap: true,
        });
        await openGenesInputViaModify(page);
        await setInputText(page, GENES_INPUT, 'BRCA1 PTEN TP53 BRCA2');
        await page.locator(SUBMIT).click();
        await waitForOncoprint(page);
        await expectOncoprintScreenshot(
            page,
            'oncoprinter-genes-reorder-all.png'
        );
    });

    test('set gene order not including all genes', async ({ page }) => {
        await submitOncoprinter(page, {
            genetic: true,
            clinical: true,
            heatmap: true,
        ...
        await openGenesInputViaModify(page);
        await setInputText(page, GENES_INPUT, 'BRCA1 PTEN');
        await page.locator(SUBMIT).click();
        await waitForOncoprint(page);
        await expectOncoprintScreenshot(
            page,
            'oncoprinter-genes-reorder-subset.png'
        );
    });

    test('set sample order including all samples', async ({ page }) => {
        await submitOncoprinter(page, {
            genetic: true,
            clinical: true,
            heatmap: true,
        });
        await openSamplesInputViaModify(page);
        await setInputText(page, SAMPLES_INPUT, SAMPLE_LIST);
        await page.locator(SUBMIT).click();
        await waitForOncoprint(page);
        await expectOncoprintScreenshot(
            page,
            'oncoprinter-samples-reorder-all.png'
        );
    });

    test('msk_impact_2017 query STK11:HOMDEL MUT', async ({ page }) => {
        await page.goto(
            '/index.do?cancer_study_id=msk_impact_2017&Z_SCORE_THRESHOLD=2' +
                ...
                '&gene_list=STK11%253A%2520HOMDEL%2520MUT&geneset_list=+' +
        ...
        await waitForOncoprint(page);
        await expectOncoprintScreenshot(page, 'msk-impact-stk11.png');
    });

    test('set sample order subset', async ({ page }) => {
        await submitOncoprinter(page, {
            genetic: true,
            clinical: true,
            heatmap: true,
        ...
        await expectOncoprintScreenshot(
            page,
            'oncoprinter-samples-reorder-subset.png'
        );
    });

    test('set gene order first, then sample order', async ({ page }) => {
        // Here we enter genes before the first submit (they're already
        // rendered in the initial form when genetic/clinical/heatmap are
        // all loaded).
        await page.goto('/oncoprinter');
        await expect(page.locator(GENETIC)).toBeVisible();
        await expect(page.locator(CLINICAL)).toBeVisible();
        await expect(page.locator(HEATMAP)).toBeVisible();
        await page.locator(GENETIC).click();
        await page.locator(CLINICAL).click();
        ...
        await expectOncoprintScreenshot(
            page,
            'oncoprinter-genes-then-samples.png'
        );
    });

    test('set sample order first, then gene order', async ({ page }) => {
        await page.goto('/oncoprinter');
        await expect(page.locator(GENETIC)).toBeVisible();
        await expect(page.locator(CLINICAL)).toBeVisible();
        await expect(page.locator(HEATMAP)).toBeVisible();
        ...
        await openGenesInputViaModify(page);
        await setInputText(page, GENES_INPUT, 'BRCA2 TP53 PTEN');
        await page.locator(SUBMIT).click();
        await waitForOncoprint(page);
        await expectOncoprintScreenshot(
            page,
            'oncoprinter-samples-then-genes.png'
        );
    });

    test('do not color by mutation type', async ({ page }) => {
        await submitOncoprinter(page, {
            genetic: true,
            clinical: true,
            heatmap: true,
        ...
        await setOncoprintMutationsMenuOpen(page, true);
        await page.locator('input[data-test="ColorByType"]').click();
        await waitForOncoprint(page);
        await setOncoprintMutationsMenuOpen(page, false);
        await expectOncoprintScreenshot(page, 'oncoprinter-no-color-type.png');
    });

    test('hide VUS', async ({ page }) => {
        await submitOncoprinter(page, {
            genetic: true,
            clinical: true,
            heatmap: true,
        ...
        await waitForOncoprint(page);
        await setOncoprintMutationsMenuOpen(page, false);
        await expectOncoprintScreenshot(page, 'oncoprinter-hide-vus.png');
    });
});
