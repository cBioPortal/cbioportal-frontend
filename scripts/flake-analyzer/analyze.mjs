...
async function main() {
    ...
    if (process.env.CIRCLE_WORKFLOW_ID) {
        const status = await getThisRunRemoteE2EStatus(
        ...
        log(
            `this run's remote_e2e status: ${status ?? 'unknown'} — proceeding with analysis`
        );
    }
    ...
    for (const flake of flakes.slice(0, MAX_FLAKES)) {
        log(
            `  analyzing ${flake.title} (${flake.numFails}f / ${flake.numPasses}p)`
        );
...
async function getThisRunRemoteE2EStatus(workflowId) {
    try {
        const wf = await getJSON(
            `https://circleci.com/api/v2/workflow/${workflowId}/job`
        );
        const remote = (wf.items ?? []).find(
            (j) => j.name === REMOTE_E2E_JOB_NAME
        );
...
async function listRecentRemoteE2EJobs(targetCount) {
    ...
    for (const p of pipelines.items ?? []) {
        ...
        for (const w of workflows.items ?? []) {
            ...
            try {
                jobs = await getJSON(
                    `https://circleci.com/api/v2/workflow/${w.id}/job`
                );
            } catch {
                continue;
            }
            for (const j of jobs.items ?? []) {
                ...
                if (['running', 'queued', 'not_run'].includes(j.status))
                    continue;
                found.push({
...
function detectFlakes(window) {
    ...
    for (const test of byTest.values()) {
        ...
        if (tail3.length === 3 && tail3.every((r) => r.status !== 'passed'))
            continue;
    ...
    flakes.sort(
        (a, b) => b.numFails - a.numFails || a.title.localeCompare(b.title)
    );
    return flakes;
}
