...
describe('Patient View Genomic Evolution tab screenshot tests', function() {
    ...
    it("pvge hover a mutation with line chart", function() {
        browser.moveToObject('div[data-test="GenomicEvolutionMutationTable"] table tbody > tr:nth-child(2)');
        const res = browser.checkElement('div[data-test="GenomicEvolutionTab"]', { hide: ['.qtip'] });
        assertScreenShotMatch(res);
    });
    it("pvge only show highlighted in line chart", function() {
        browser.click('input[data-test="VAFOnlyHighlighted"]');
        const res = browser.checkElement('svg[data-test="VAFLineChart"]', { hide: ['.qtip'] });
        assertScreenShotMatch(res);
    });
    ...
    it("pvge add a mutation to line chart", function() {
        browser.click('div[data-test="GenomicEvolutionMutationTable"] table tbody > tr:nth-child(7)');
        const res = browser.checkElement('svg[data-test="VAFLineChart"]', { hide: ['.qtip'] });
        assertScreenShotMatch(res);
    });
    ...
    it("pvge hover a mutation with heatmap", function() {
        browser.moveToObject('div[data-test="GenomicEvolutionMutationTable"] table tbody > tr:nth-child(9)');
        const res = browser.checkElement('div[data-test="GenomicEvolutionTab"]', { hide: ['.qtip'] });
        assertScreenShotMatch(res);
    });
    it("pvge uncluster heatmap", function() {
        ...
        const res = checkElementWithMouseDisabled('div#MutationHeatmap', 0, { hide:[".qtip", '.dropdown-menu'] });
        assertScreenShotMatch(res);
    });
