...
function waitForAndCheckPlotsTab() {
    ...
    var res = await browser.checkElement('div[data-test="PlotsTabEntireDiv"]');
...
function runResultsTestSuite(prefix, options = {}) {
    it(`${prefix} render the oncoprint`, async function() {
        ...
        var res = await browser.checkElement('.oncoprintContainer'); // just hide the controls bc for some reason they keep showing up transparent in this test only
        assertScreenShotMatch(res);
    });

    // can't get it to pass reliably
    it.skip(`${prefix} igv_tab tab`, async function() {
        $('a.tabAnchor_cnSegments').click();
        $('#cnSegmentsFrame').waitForExist({ timeout: 20000 });
        var res = await browser.checkElement('.cnSegmentsMSKTabs', {
            hide: ['.qtip'],
        });
        assertScreenShotMatch(res);
    });
    ...
    it(`${prefix} mutex tab`, async function() {
        ...
        var res = await browser.checkElement('[data-test="mutualExclusivityTabDiv"]');
        assertScreenShotMatch(res);
    });
    ...
    it(`${prefix} mutation tab`, async function() {
        ...
        var res = await browser.checkElement('[data-test="mutationsTabDiv"]', '', {
    ...
    it(`${prefix} coexpression tab`, async function() {
        ...
        var res = await browser.checkElement('[data-test="coExpressionTabDiv"]');
        assertScreenShotMatch(res);
    });
    ...
    it(`${prefix} comparison tab mrna enrichments`, async function() {
        ...
        var res = await browser.checkElement('div[data-test="ComparisonTabDiv"]');
        assertScreenShotMatch(res);
    });
    ...
    it.skip(`${prefix} data_download tab`, async function() {
        ...
        var res = await browser.checkElement('[data-test="downloadTabDiv"]', '', {
            hide: ['.qtip'],
        });
        assertScreenShotMatch(res);
    });
...
describe('patient view page screenshot test', function() {
    it('patient view lgg_ucsf_2014 P04', async function() {
        ...
        var res = await browser.checkElement('#mainColumn');
        assertScreenShotMatch(res);
    });
    ...
    it('patient view with 0 mutations msk_impact_2017 P-0000053-T01-IM3', async function() {
        ...
        // take screenshot
        var res = await browser.checkElement('#mainColumn', '', { hide: ['.qtip'] });
        assertScreenShotMatch(res);
    });
...
//     });
//
//     it('should show error message for 400 query', async function() {
//         $('.errorScreen').waitForExist();
//     });
//
//     it('should allow return to homepage after error message', async function() {
//         $('.errorLogo').click();
...
//     it('should show error message for wrong sample', async function() {
//         var url = `${CBIOPORTAL_URL}/patient?sampleId=not-a-sample&studyId=msk_impact_2017`;
//         goToUrlAndSetLocalStorage(url);
//
//         $('.errorScreen').waitForExist();
//         var res = await browser.checkElement('.errorScreen', {
//             hide: ['.form-group'],
//         });
//         assertScreenShotMatch(res);
//     });
//
//     it('should show error message for wrong patient', async function() {
