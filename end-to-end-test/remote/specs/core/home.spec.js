...
describe('auto-selecting needed profiles for oql in query form', () => {
    ...
    it('gives a submit error if protein oql is inputted and no protein profile is available for the study', () => {
        browser.waitForExist('.studyItem_nsclc_mskcc_2018', 20000);
        browser.click('.studyItem_nsclc_mskcc_2018');
        clickQueryByGeneButton();
...
describe.skip('results page quick oql edit', () => {
    before(() => {
        goToUrlAndSetLocalStorage(
            `${CBIOPORTAL_URL}/results/oncoprint?cancer_study_list=nsclc_mskcc_2018&Z_SCORE_THRESHOLD=2.0&RPPA_SCORE_THRESHOLD=2.0&profileFilter=mutations&case_set_id=nsclc_mskcc_2018_sequenced&gene_list=CDKN2A%2520MDM2%2520MDM4%2520TP53&geneset_list=%20&tab_index=tab_visualize&Action=Submit`
        );
    });
