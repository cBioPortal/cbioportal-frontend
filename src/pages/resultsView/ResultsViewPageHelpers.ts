...
export function populateSampleSpecificationsFromVirtualStudies(
    ...
    samplesSpecification = _.filter(
        samplesSpecification,
        spec => !virtualStudiesKeyedById[spec.studyId]
    );

    const allVirtualStudySampleSpecs = _.flatMapDeep(
    ...
    samplesSpecification = samplesSpecification.concat(
        allVirtualStudySampleSpecs as SamplesSpecificationElement[]
    );
...
export function parseSamplesSpecifications(
    ...
    if (query.case_ids && query.case_ids.length > 0) {
        ...
        samplesSpecification = query.sample_list_ids
            .split(',')
