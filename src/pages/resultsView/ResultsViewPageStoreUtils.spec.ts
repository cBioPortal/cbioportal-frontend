...
describe('ResultsViewPageStoreUtils', () => {
    ...
    describe('getRNASeqProfiles', () => {
        it('properly recognizes expression profile based on patterns in id', () => {
            ...
            assert.isTrue(
                isRNASeqProfile('acc_tcga_rna_seq_v2_mrna', 2),
                'matches seq v2 id'
            );
            assert.isFalse(
                isRNASeqProfile('acc_tcga_rna_seq_v2_mrna', 1),
                'fails if versions is wrong'
            );
            assert.isTrue(
                isRNASeqProfile(
...
describe('getSampleAlteredMap', () => {
    ...
    it('handles different type of gene tracks correctly', () => {
        ...
        const expectedResult = {
            RAS: [true, false, true, false, false, true, false, false],
            'SMAD4 / RAN': [false, true, true, false, true, true, false, false],
            'SMAD4: MUT': [false, true, true, false, true, true, false, false],
            KRAS: [true, false, true, false, false, true, false, false],
        };
        assert.deepEqual(
            ret['KRAS'],
            expectedResult['KRAS'],
