...
export function isRNASeqProfile(profileId: string, version: number): boolean {
    ...
    return RegExp(
        `rna_seq_${ver}mrna$|pan_can_atlas_2018_rna_seq_${ver}mrna_median$`
    ).test(profileId);
}
...
export function makeEnrichmentDataPromise<
    T extends { hugoGeneSymbol: string; pValue: number; qValue?: number }
>(params: {
    ...
    return remoteData({
        await: () =>
            params.await().concat([params.store.selectedMolecularProfiles]),
        invoke: async () => {
            ...
            if (profile) {
                ...
                if (
                    params.store.selectedMolecularProfiles.result!.findIndex(
                        x => x.molecularProfileId === profile.molecularProfileId
                    ) > -1
                ) {
