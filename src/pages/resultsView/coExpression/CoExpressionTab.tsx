...
> {
    ...
    @computed get selectedMolecularProfile(): MolecularProfile | undefined {
        if (
            !this._selectedMolecularProfile &&
    ...
    @computed get selectedEntrezGeneId(): number | undefined {
        if (
            this._selectedEntrezGeneId === undefined &&
    ...
    @computed public get onSelectDataSet() {
        if (this.props.store.molecularProfileIdToMolecularProfile.isComplete) {
            return (option: any) => {
                this._selectedMolecularProfile = this.props.store.molecularProfileIdToMolecularProfile.result[
                    option.value
                ];
            };
        } else {
    ...
    private coExpressionCache: CoExpressionCache = new CoExpressionCache(
        q => ({
            invoke: () => {
                ...
                const dataQueryFilter = this.props.store.studyToDataQueryFilter
                    .result![q.molecularProfile.studyId];
                if (dataQueryFilter) {
                    ...
                    return internalClient.fetchCoExpressionsUsingPOST({
                        molecularProfileId:
                            q.molecularProfile.molecularProfileId,
                ...
                >
                    ...
                    >
                        <MolecularProfileSelector
                            ...
                            value={
                                this.selectedMolecularProfile.molecularProfileId
                            }
    ...
    @bind
    private header() {
        return <div style={{ marginBottom: 20 }}>{this.dataSetSelector}</div>;
    }
        ...
        if (
            ...
            return (
                <div>
                    ...
                    >
                        {this.props.store.genes.result!.map(
                            (gene: Gene, i: number) => {
                                return (
                                    <MSKTab
                                        key={i}
