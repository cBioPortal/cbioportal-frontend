...
// need to do this b/c this code was written with old version of lodash
_.pluck = _.map;

// handles fact that we don't have collapse plugin on webpack version of jquery
                            ...
                            _.each(_non_mut_study_groups, function (_non_mut_study_group) {
                                ...
                                _.each(_sequenced_sample_lists, function (_sequenced_sample_list) {
                                    if (_sequenced_sample_list.study_id === _non_mut_study_group[0].study_id) {
                                        ...
                                        _.each(_non_mut_study_group, function (_non_mut_obj) {
                                            if (_.includes(_sequenced_sample_ids, _non_mut_obj.sample_id)) {
...
var renderPlots = function(_d) {
    ...
    $("#cc_plots_box").empty();
    window.Plotly.newPlot('cc_plots_box', data4render, layout, {showLink: false});
...
export function init(cancer_study_ids, genes) {

                if (cancer_study_ids.length > 0) {

                    if ($("#cc_plots_gene_list option").length === 0) {
                        _.each(genes, function (_gene) {
                            $("#cc_plots_gene_list").append(
                                "<option value='" + _gene + "'>" + _gene + "</option>");
                        });
                    }

                    $.when(getGeneticProfiles_(cancer_study_ids)).then(function(_profiles) {
                        ...
                        // data fetching
                        $.when(fetchProfileData_(cancer_study_ids)).then(function(_d) {
...
export function getGeneticProfiles_(_sids) {
