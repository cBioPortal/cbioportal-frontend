...
export function getPseudoOqlSummaryByAlterationTypes(
    ...
    const alteratedPseudoOqlSummarys = _.chain(alterationTypes)
        ...
        .filter(summary =>
            summary ? summary.summaryClass === styles.alterationSpan : false
        )
        .value();
    const alteratedPseudoOqlSummaryContent = _.map(
        alteratedPseudoOqlSummarys,
        (summary: PseudoOqlSummary) => summary.summaryContent
    ).join(',');
    return {
        summaryContent: alteratedPseudoOqlSummaryContent,
        summaryClass: styles.alterationSpan,
    };
}
...
> {
    public render() {
        ...
        const geneSet: {
            [x: string]: OQLLineFilterOutput<AnnotatedExtendedAlteration>;
        } = {};
