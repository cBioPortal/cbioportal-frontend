...
export function generateSampleAlterationDataByGene(
    ...
    _.values(unfilteredCaseAggregatedData.samples).forEach(alterations => {
        alterations.forEach(alteration => {
            const key = `${alteration.gene.hugoGeneSymbol}_${
    ...
    if (caseAggregatedDataByOQLLine && genePanelInformation) {
        ...
        caseAggregatedDataByOQLLine.forEach(data => {
            ...
            geneticTrackData.forEach(datum => {
                ...
                const sampleId =
                    datum.sample ||
                    (sampleIndex[datum.uid]
                        ? sampleIndex[datum.uid].sampleId
                        : '');
                ...
                caseAlterationData[key].oqlData[
                    data.oql.oql_line
                ] = generatedOqlData;
                updateOqlData(
                    datum,
                    caseAlterationData[key].oqlData[data.oql.oql_line],
                    molecularProfileIdToMolecularProfile
                );
