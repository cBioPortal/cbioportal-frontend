...
type ExpressionEnrichmentTableColumn = Column<ExpressionEnrichmentRow> & {
    order?: number;
...
> {
    @observable protected _columns: {
        [columnEnum: number]: ExpressionEnrichmentTableColumn;
    };
    ...
    protected generateColumns(): {
            ...
            render: (d: ExpressionEnrichmentRow) => (
                <div style={{ display: 'flex' }}>
                    <Checkbox
                        ...
                        onChange={() => this.checkboxChange(d.hugoGeneSymbol)}
                        title={
                            d.disabled ? 'This is one of the query genes' : ''
                        }
                    />
        ...
        columns[ExpressionEnrichmentTableColumnType.MEAN_IN_ALTERED] = {
            ...
            render: (d: ExpressionEnrichmentRow) => (
                <span>{d.meanExpressionInAlteredGroup.toFixed(2)}</span>
            ),
            tooltip: (
                <span>
                    Mean log2 expression of the listed gene in samples that have
                    alterations in the query gene(s).
                </span>
            ),
            sortBy: (d: ExpressionEnrichmentRow) =>
                d.meanExpressionInAlteredGroup,
            download: (d: ExpressionEnrichmentRow) =>
                d.meanExpressionInAlteredGroup.toFixed(2),
        };

        columns[ExpressionEnrichmentTableColumnType.MEAN_IN_UNALTERED] = {
            name: 'μ in unaltered group',
            render: (d: ExpressionEnrichmentRow) => (
                <span>{d.meanExpressionInUnalteredGroup.toFixed(2)}</span>
            ),
            tooltip: (
                <span>
                    Mean log2 expression of the listed gene in samples that do
                    not have alterations in the query gene(s).
                </span>
            ),
            sortBy: (d: ExpressionEnrichmentRow) =>
                d.meanExpressionInUnalteredGroup,
            download: (d: ExpressionEnrichmentRow) =>
                d.meanExpressionInUnalteredGroup.toFixed(2),
        };
        ...
        columns[
            ...
            render: (d: ExpressionEnrichmentRow) => (
                <span>{d.standardDeviationInAlteredGroup.toFixed(2)}</span>
            ),
            tooltip: (
                <span>
            ...
            sortBy: (d: ExpressionEnrichmentRow) =>
                d.standardDeviationInAlteredGroup,
            download: (d: ExpressionEnrichmentRow) =>
                d.standardDeviationInAlteredGroup.toFixed(2),
        };
        ...
        columns[
            ...
            render: (d: ExpressionEnrichmentRow) => (
                <span>{d.standardDeviationInUnalteredGroup.toFixed(2)}</span>
            ),
            tooltip: (
                <span>
                    Standard deviation of log2 expression of the listed gene in
            ...
            sortBy: (d: ExpressionEnrichmentRow) =>
                d.standardDeviationInUnalteredGroup,
            download: (d: ExpressionEnrichmentRow) =>
                d.standardDeviationInUnalteredGroup.toFixed(2),
        };
        ...
        columns[ExpressionEnrichmentTableColumnType.LOG_RATIO] = {
            ...
            tooltip: (
                <span>
                    Log2 of ratio of (unlogged) mean in altered to (unlogged)
                    mean in unaltered
                </span>
            ),
