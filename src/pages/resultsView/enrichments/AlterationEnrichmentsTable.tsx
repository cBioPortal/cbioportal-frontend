...
type AlterationEnrichmentTableColumn = Column<AlterationEnrichmentRow> & {
    order?: number;
...
> {
    @observable protected _columns: {
        [columnEnum: number]: AlterationEnrichmentTableColumn;
    };
    ...
    protected generateColumns(): {
        ...
        columns[AlterationEnrichmentTableColumnType.PERCENTAGE_IN_ALTERED] = {
            ...
            render: (d: AlterationEnrichmentRow) => (
                <span>
                    {formatPercentage(d.alteredCount, d.alteredPercentage)}
                </span>
            ),
            headerRender: (name: string) => (
                <span style={{ display: 'inline-block', width: 165 }}>
                    {name}
                </span>
            ),
            tooltip: (
                <span>
                    Number (percentage) of samples that have alterations in the
                    query gene(s) that also have {this.props.alterationType} in
                    the listed gene.
                </span>
            ),
        ...
        columns[AlterationEnrichmentTableColumnType.PERCENTAGE_IN_UNALTERED] = {
            ...
            render: (d: AlterationEnrichmentRow) => (
                <span>
                    {formatPercentage(d.unalteredCount, d.unalteredPercentage)}
                </span>
            ),
            headerRender: (name: string) => (
                <span style={{ display: 'inline-block', width: 165 }}>
            ...
            tooltip: (
                <span>
                    Number (percentage) of samples that do not have alterations
                    in the query gene(s) that have {this.props.alterationType}{' '}
                    in the listed gene.
                </span>
            ),
        ...
        columns[AlterationEnrichmentTableColumnType.LOG_RATIO] = {
            ...
            tooltip: (
                <span>
                    Log2 based ratio of (pct in altered / pct in unaltered)
                </span>
            ),
        ...
        columns[AlterationEnrichmentTableColumnType.TENDENCY] = {
            ...
            render: (d: AlterationEnrichmentRow) => (
                <div className={styles.Tendency}>
                    {calculateAlterationTendency(Number(d.logRatio))}
                    {d.qValue < 0.05 ? (
                        <Badge
                            style={{
