    ...
    return alterationEnrichments
        ...
        .map(alterationEnrichment => {
            return {
                ...
                y: -Math.log10(alterationEnrichment.pValue),
...
export function getAlterationRowData(
    ...
    return alterationEnrichments.map(alterationEnrichment => {
        return {
            ...
            alteredPercentage:
                (alterationEnrichment.alteredCount / totalAltered) * 100,
            unalteredCount: alterationEnrichment.unalteredCount,
            unalteredPercentage:
                (alterationEnrichment.unalteredCount / totalUnaltered) * 100,
    ...
    return expressionEnrichments.map(expressionEnrichment => {
        return {
            ...
            meanExpressionInAlteredGroup:
                expressionEnrichment.meanExpressionInAlteredGroup,
            meanExpressionInUnalteredGroup:
                expressionEnrichment.meanExpressionInUnalteredGroup,
            standardDeviationInAlteredGroup:
                expressionEnrichment.standardDeviationInAlteredGroup,
            standardDeviationInUnalteredGroup:
                expressionEnrichment.standardDeviationInUnalteredGroup,
            logRatio:
                expressionEnrichment.meanExpressionInAlteredGroup -
                expressionEnrichment.meanExpressionInUnalteredGroup,
            pValue: expressionEnrichment.pValue,
...
export function getFilteredData(
    data: Pick<
        ExpressionEnrichmentRow,
        'logRatio' | 'qValue' | 'hugoGeneSymbol'
    >[],
    negativeLogFilter: boolean,
    positiveLogFilter: boolean,
    ...
    return data.filter(alterationEnrichment => {
        ...
        if (selectedGenes) {
            result =
                result &&
                selectedGenes.includes(alterationEnrichment.hugoGeneSymbol);
        }
        return result;
    });
}
