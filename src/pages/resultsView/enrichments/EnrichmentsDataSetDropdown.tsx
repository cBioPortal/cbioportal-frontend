...
export interface IEnrichmentsDataSetDropdownProps {
    ...
    selectedProfileByStudyId: { [id: string]: Pick<MolecularProfile, "molecularProfileId"> };
...
export default class EnrichmentsDataSetDropdown extends React.Component<IEnrichmentsDataSetDropdownProps, {}> {
    ...
    private change(selectedStudyId: string, o: any) {
        ...
        let updatedStudyMolecularProfileMap = _.mapValues(this.props.selectedProfileByStudyId, (molecularProfile, studyId) => {
    ...
    @computed private get showEnrichmentsDataSetDropdown() {
        if (this.props.dataSets.isComplete) {
            let molecularProfilesMap = _.groupBy(this.props.dataSets.result!, profile => profile.studyId);
            return !!_.find(molecularProfilesMap, molecularProfiles => molecularProfiles.length > 1);
        }
        return false;
    }

    public render() {
        if (this.props.dataSets.isPending || (this.props.molecularProfileIdToProfiledSampleCount && this.props.molecularProfileIdToProfiledSampleCount.isPending)) {
            ...
            return _.map(studyProfilesMap, (molecularProfiles, studyId) => (
                <div className={styles.DataSet}>
                    <div style={{ display: "flex", alignItems: "center" }}>
                        <strong>{includeStudyName ? this.studiesMap[studyId].name + " " : ""}Data Set:</strong>
                        <div style={{ display: "inline-block", marginLeft: 5, width: 400 }}>
                            <MolecularProfileSelector
                                value={this.props.selectedProfileByStudyId[studyId].molecularProfileId}
                                molecularProfiles={molecularProfiles}
                                molecularProfileIdToProfiledSampleCount={this.props.molecularProfileIdToProfiledSampleCount ? this.props.molecularProfileIdToProfiledSampleCount.result : undefined}
