...
> {
    ...
    public render() {
        ...
        if (this.props.store.alteredSampleKeys.result!.length === 0) {
            return (
                <div className={'alert alert-info'}>
                    No queried samples are altered, therefore this calculation
                    cannot be performed.
                </div>
            );
        }
        ...
        if (this.props.store.unalteredSampleKeys.result!.length === 0) {
            return (
                <div className={'alert alert-info'}>
                    All queried samples are altered, therefore this calculation
                    cannot be performed.
                </div>
            );
        }
        ...
        return (
            <div data-test="enrichmentsTabDiv">
                ...
                >
                    ...
                    {this.props.store.copyNumberEnrichmentProfiles.result!
                        .length > 0 && (
                        <MSKTab id="copynumber" linkText="Copy-number">
                            <CopyNumberEnrichmentsTab
                                store={this.props.store}
                            />
                        </MSKTab>
                    )}
                    {this.props.store.mRNAEnrichmentProfiles.result!.length >
                        0 &&
                    ...
                    {this.props.store.proteinEnrichmentProfiles.result!.length >
                        0 &&
                        this.props.store.studies.result!.length === 1 && (
                            <MSKTab id="protein" linkText="Protein">
                                <ProteinEnrichmentsTab
                                    store={this.props.store}
                                />
                            </MSKTab>
                        )}
