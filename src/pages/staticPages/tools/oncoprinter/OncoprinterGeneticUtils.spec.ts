...
import { initDriverAnnotationSettings, parseGeneticInput } from './OncoprinterGeneticUtils';
...
describe('OncoprinterGeneticUtils', () => {
    ...
    describe('parseGeneticInput', () => {
        it('parses fusion command correctly', () => {
            assert.deepEqual(parseGeneticInput('sample_id TP53 FUSION FUSION'), {
                status: 'complete',
                result: [
                    {
                        sampleId: 'sample_id',
        ...
        it('throws an error if fusion is not specified correctly', () => {
            try {
                const parsed = parseGeneticInput('sample_id TP53 FUSION apsoidjfpaos');
                assert(false);
            } catch (e) {}
        });
        it('parses germline mutation correctly', () => {
            assert.deepEqual(parseGeneticInput('sampleid	BRCA1	Q1538A	MISSENSE_GERMLINE'), {
                status: 'complete',
                result: [
                    {
                        sampleId: 'sampleid',
        ...
        it('parses driver mutation correctly', () => {
            assert.deepEqual(parseGeneticInput('sampleid	BRCA1	Q1538A	TRUNC_DRIVER'), {
                status: 'complete',
                result: [
                    {
                        sampleId: 'sampleid',
        ...
        it('parses germline & driver mutation correctly', () => {
            assert.deepEqual(parseGeneticInput('sampleid	BRCA1	Q1538A	MISSENSE_GERMLINE_DRIVER'), {
                status: 'complete',
                result: [
                    {
                        sampleId: 'sampleid',
