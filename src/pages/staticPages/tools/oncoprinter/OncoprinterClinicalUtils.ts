...
import {OncoprinterGeneticTrackDatum, OncoprinterGeneticTrackDatum_Data} from "./OncoprinterGeneticUtils";
import {CustomTrackOption} from "oncoprintjs";
import {ClinicalTrackDatum, ClinicalTrackSpec} from "../../../../shared/components/oncoprint/Oncoprint";
...
type AttributeSpec = { clinicalAttributeName:string, datatype:ClinicalTrackDataType };

type OncoprinterClinicalTrackDatum = Pick<ClinicalTrackDatum, "attr_id"|"uid"|"attr_val_counts"|"na"> & {sample:string, attr_val:string|number};

const ATTRIBUTE_REGEX = /^((?:[^\(\)])+)(?:\(([^\(\)]+)\))?$/;

enum ClinicalTrackDataType {
    NUMBER="number", LOG_NUMBER="lognumber", STRING="string"
}
    ...
    try {
        ...
        const result = lines.map((line, lineIndex)=>{
            ...
            if (line.length === attributes.length + 1) {
                ...
                throw new Error(`${errorPrefix}data lines must have ${attributes.length + 1} columns, the same number as in the header.`);
            }
        });
        return {
            status: "complete",
            result: { headers: attributes, data: result.filter(x=>!!x) as OncoprinterClinicalInputLine[]},
            error:undefined
        };
    } catch (e) {
        return {
...
export function getClinicalOncoprintData(
    ...
    const ret:{[clinicalAttributeName:string]:OncoprinterClinicalTrackDatum[]} = {};
    ...
    for (const line of parsedLines) {
        for (let i=0; i<attributes.length; i++) {
            ...
            const value = attributes[i].datatype === ClinicalTrackDataType.STRING ? rawValue : parseFloat(rawValue);
...
export function getClinicalTracks(
    ...
    return attributes.map(attr=>{
        ...
        return {
            ...
            description:"",
            numberRange: attr.datatype === ClinicalTrackDataType.STRING ? undefined : getNumberRange(data),
