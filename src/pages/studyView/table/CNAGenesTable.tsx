...
export class CNAGenesTable extends React.Component<ICNAGenesTablePros, {}> {
    ...
    get tableColumns() {
        return [
                        ...
                        <div className={classnames(styles.displayFlex)}>
                            ...
                            <If condition={!_.isUndefined(data.qValue)}>
                                ...
                                >
                                    <span>
                                        <img
                                            src={require('./images/gistic.png')}
                                            className={styles.gistic}
                                        />
                                    </span>
                                </DefaultTooltip>
                            </If>
            ...
            },
            {
                ...
                tooltip: (
                    <span>
                        Number of samples with the listed copy number alteration
                    </span>
                ),
                headerRender: () => {
                    return (
                ...
                filter: (data: CopyNumberCountByGene, filterString: string) => {
                    return _.toString(data.countByEntity).includes(
                        filterString
                    );
                },
                width: 75,
            },
            {
                ...
                tooltip: (
                    <span>
                        Percentage of samples with the listed copy number
                        alteration
                    </span>
                ),
                headerRender: () => {
                ...
                render: (data: CopyNumberCountByGene) => (
                    ...
                    >
                        {getFrequencyStr(data.frequency)}
                    </span>
                ),
                sortBy: (data: CopyNumberCountByGene) => data.frequency,
                defaultSortDirection: 'desc' as 'desc',
                filter: (data: CopyNumberCountByGene, filterString: string) => {
                    return _.toString(getFrequencyStr(data.frequency)).includes(
                        filterString
                    );
                },
                width: 70,
    ...
    togglePreSelectRow(entrezGeneId: number, alteration: number) {
        let record: CNAGenesTableUserSelectionWithIndex | undefined = _.find(
            this.preSelectedRows,
            (row: CNAGenesTableUserSelectionWithIndex) =>
                row.entrezGeneId === entrezGeneId &&
                row.alteration === alteration
        ...
        if (_.isUndefined(record)) {
            ...
            let datum: CopyNumberCountByGene | undefined = _.find(
                this.props.promise.result,
                (row: CopyNumberCountByGene, index: number) => {
                    let exist =
                        row.entrezGeneId === entrezGeneId &&
                        row.alteration === alteration;
                    if (exist) {
    ...
    isSelectedRow(data: CopyNumberCountByGene) {
        return !_.isUndefined(
            _.find(_.union(this.selectedRows, this.preSelectedRows), function(
                row
            ) {
                return (
