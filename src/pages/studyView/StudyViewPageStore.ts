...
export class StudyViewPageStore {
    ...
    constructor() {
        this.reactionDisposers.push(
            reaction(
                () => this.filters,
                () => this.clearAnalysisGroupsSettings()
            )
        ...
        SPECIAL_CHARTS.forEach(chartMeta => {
            ...
            const chartType =
                this.chartsType.get(uniqueKey) || chartMeta.chartType;
    ...
    updateStoreByFilters(filters: Partial<StudyViewFilter>) {
        ...
        if (
            _.isArray(filters.mutatedGenes) &&
            filters.mutatedGenes.length > 0
        ) {
    ...
    get containerWidth(): number {
        return (
            this.studyViewPageLayoutProps.cols *
        ...
        invoke: () => {
            if (this.analysisGroupsSettings.clinicalAttribute) {
                return Promise.resolve(
                    this.sampleToClinicalAnalysisGroup.result!
                );
            } else {
                    ...
                    _.reduce(
                        ...
                        (map, nextSample) => {
                            ...
                            if (sampleKey in selectedSamplesMap) {
                                ...
                                map[
                                    sampleKey
                                ] = UNSELECTED_ANALYSIS_GROUP_VALUE;
                            }
                            return map;
                        },
    ...
    readonly patientToAnalysisGroup = remoteData<{
        ...
        invoke: () => {
            if (this.analysisGroupsSettings.clinicalAttribute) {
                return Promise.resolve(
                    this.patientToClinicalAnalysisGroup.result!
                );
            } else {
        ...
        invoke: () => {
            ...
            if (data.patientAttribute) {
                ...
                return Promise.resolve(
                    makePatientToClinicalAnalysisGroup(
                        this.selectedSamples.result!,
                        this.clinicalAnalysisGroupsData.result!
                            .caseToAnalysisGroup
                    )
                );
            }
    ...
    addCNAGeneFilters(filters: CopyNumberAlterationIdentifier[]) {
        ...
        filters.forEach(
            filter =>
                (this.geneMapCache[filter.entrezGeneId] = filter.hugoGeneSymbol)
        );
        this._cnaGeneFilter = [
            ...this._cnaGeneFilter,
            {
        ...
        if (!visible) {
            switch (chartMeta.chartType) {
                ...
                default:
                    ...
                    this._clinicalDataEqualityFilterSet.delete(
                        chartMeta.uniqueKey
                    );
                    this._clinicalDataIntervalFilterSet.delete(
                        chartMeta.uniqueKey
                    );
    ...
    get filters(): StudyViewFilter {
        ...
        let _sampleIdentifiers: SampleIdentifier[] = _.chain(
            this._chartSampleIdentifiersFilterSet.values()
        ) // for some reason its a lot faster with _.chain than just using _.intersectionWith
            .intersectionWith(((a: SampleIdentifier, b: SampleIdentifier) => {
            ...
            >({
                ...
                invoke: async () => {
                    ...
                    if (
                        ...
                        if (
                            this._clinicalDataEqualityFilterSet.has(
                                uniqueKey
                            ) ||
                            this.isInitialFilterState
                        ) {
    ...
    public getClinicalDataBin(chartMeta: ChartMeta) {
        ...
        if (!this.clinicalDataBinPromises.hasOwnProperty(uniqueKey)) {
            ...
            this.clinicalDataBinPromises[uniqueKey] = remoteData<DataBin[]>({
                ...
                invoke: async () => {
                    const clinicalDataType = chartMeta.clinicalAttribute!
                        .patientAttribute
                        ? 'PATIENT'
                        : 'SAMPLE';
            ...
            clinicalAttributes.forEach((obj: ClinicalAttribute) => {
                if (obj.datatype === 'NUMBER') {
                    ...
                    let filter = {
                        ...
                        clinicalDataType: obj.patientAttribute
                            ? 'PATIENT'
                            : 'SAMPLE',
            ...
            (acc, [chartUniqueKey, visible]) => {
                if (visible && this.chartMetaSet[chartUniqueKey]) {
                    ...
                    if (dimension !== undefined) {
                        ...
                        chartMeta.dimension = STUDY_VIEW_CONFIG.layout
                            .dimensions[chartMeta.chartType] || { w: 1, h: 1 };
                    }
                    acc.push(chartMeta);
                }
                return acc;
    ...
    updateChartStats() {
        ...
        if (!_.isEmpty(this.mutationProfiles.result)) {
            const mutatedGeneMeta = _.find(
                this.chartMetaSet,
                chartMeta =>
                    chartMeta.uniqueKey === UniqueKey.MUTATED_GENES_TABLE
            );
        ...
        if (newChartType === ChartTypeEnum.TABLE) {
            ...
            if (data !== undefined) {
                this.chartsDimension[
                    attr.uniqueKey
                ] = this.getTableDimensionByNumberOfRecords(
                    data.result!.length
                );
            }
            ...
            _.each(this.survivalData.result, clinicalDataList => {
                ...
                if (clinicalDataList.length > 0) {
                    ...
                    _.each(uniqueClinicalAttributeIds, id => {
                        row.push(
                            keyed[id] ? keyed[id].value || NA_DATA : NA_DATA
                        );
                    });
                }
    ...
    public async getMutatedGenesDownloadData() {
        if (this.mutatedGeneData.result) {
            let data = [
                ['Gene', 'MutSig(Q-value)', '# Mut', '#', 'Freq'].join('\t'),
            ];
    ...
    public async getGenesCNADownloadData() {
        if (this.cnaGeneData.result) {
            ...
            _.each(this.cnaGeneData.result, function(
                record: CopyNumberCountByGene
            ) {
                data.push(
                    [
                        record.hugoGeneSymbol,
    ...
    readonly mutationCountVsCNADensityData = remoteData<{
        ...
        invoke: async () => {
            if (
                ...
                if (filter) {
                    yAxisBinCount = Math.min(
                        yAxisBinCount,
                        Math.floor(filter.yEnd)
                    );
                }
        ...
        if (values.length > 0) {
            ...
            switch (chartMeta.uniqueKey) {
                case UniqueKey.CANCER_STUDIES: {
                    filteredSampleIdentifiers = filteredSampleIdentifiers.concat(
                        getFilteredSampleIdentifiers(
                ...
                default:
                    filteredSampleIdentifiers = _.reduce(
                        this._customChartsSelectedCases.get(
                            chartMeta.uniqueKey
                        ),
                        (acc, next) => {
