...
export interface IChartContainerProps {
    ...
    setAnalysisGroupsSettings: (
        attribute: ClinicalAttribute,
        grp: ReadonlyArray<AnalysisGroup>
    ) => void;
...
export class ChartContainer extends React.Component<IChartContainerProps, {}> {
    ...
    get analysisGroupsPossible() {
        return (
            !!this.props.analysisGroupsPossible &&
            (this.chartType === ChartTypeEnum.PIE_CHART ||
                this.chartType === ChartTypeEnum.TABLE) &&
    ...
    setAnalysisGroups() {
        if (this.analysisGroupsPossible) {
            this.props.setAnalysisGroupsSettings(
                this.props.chartMeta.clinicalAttribute!,
                this.props.promise.result as ClinicalDataCountWithColor[]
            );
        }
    }
    ...
    @computed get survivalChartData() {
        ...
        if (
            ...
            (!this.props.patientKeysWithNAInSelectedClinicalData ||
                this.props.patientKeysWithNAInSelectedClinicalData.isComplete)
        ) {
            const survivalData = _.find(
            ...
            return makeSurvivalChartData(
                ...
                this.props.patientKeysWithNAInSelectedClinicalData &&
                    this.props.patientKeysWithNAInSelectedClinicalData.result!
            );
        } else {
            return undefined;
        }
                ...
                return () => (
                    <PieChart
                        width={getWidthByDimension(
                            this.props.chartMeta.dimension,
                            this.borderWidth
                        )}
                ...
                if (this.survivalChartData) {
                    ...
                    return () => (
                        <SurvivalChart
                            ...
                            analysisClinicalAttribute={
                                this.props.analysisGroupsSettings
                                    .clinicalAttribute
                            }
                            naPatientsHiddenInSurvival={
                                this.naPatientsHiddenInSurvival
                            }
                            ...
                            toggleSurvivalHideNAPatients={
                                this.toggleSurvivalHideNAPatients
                            }
    ...
    @computed get isAnalysisTarget() {
        return (
            this.props.analysisGroupsSettings.clinicalAttribute &&
            this.props.chartMeta.clinicalAttribute &&
            this.props.analysisGroupsSettings.clinicalAttribute
                .clinicalAttributeId ===
                this.props.chartMeta.clinicalAttribute.clinicalAttributeId
        );
    }
        ...
        return _.reduce(
            ...
            (acc, next) => {
                ...
                if (
                    !(
                        this.chartType === ChartTypeEnum.TABLE &&
                        _.includes(['SVG', 'PDF'], next.type)
                    )
                ) {
                    acc.push({
                        type: next.type,
                        initDownload: next.initDownload
                            ? next.initDownload
                            : this.handlers.defaultDownload[next.type],
                    });
                }
                return acc;
            },
