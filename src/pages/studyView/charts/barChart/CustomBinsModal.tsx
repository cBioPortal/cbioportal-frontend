...
export default class CustomBinsModal extends React.Component<ICustomBinsProps, {}> {
    ...
    render() {
        return <Modal bsSize={'small'} show={this.props.show} onHide={this.props.onHide} keyboard>
            ...
            <Modal.Body>
                ...
                />
                {!this.contentIsValid && (<div className="alert alert-danger" role="alert"
                                               style={{marginTop: '10px', marginBottom: '0'}}>Invalid bins</div>)}
            </Modal.Body>
            <Modal.Footer>
                <button className='btn btn-primary btn-sm' onClick={this.updateCurrentBinsValue}
                        disabled={!this.contentIsValid} style={{marginTop: '10px', marginBottom: '0'}}>Update
                </button>
            </Modal.Footer>
        </Modal>
    }
