...
describe("VAFLineChartUtils", () => {
    describe("computeRenderData", ()=>{
        ...
        function checkResult(actual:{ grayPoints:IPoint[], lineData:IPoint[][]}, expected:{grayPoints:IPoint[], lineData:IPoint[][]}, messagePrefix?:string) {
            // gray points in any order
            roughlyDeepEqualPoints(actual.grayPoints, expected.grayPoints, `${messagePrefix||""}grayPoints`);

            // lines in any order, but the actual order of each line internally does matter
            assert.equal(actual.lineData.length, expected.lineData.length, `${messagePrefix||""}lineData length`);
            const mutationKeyToLineData = _.keyBy(actual.lineData, d=>generateMutationIdByGeneAndProteinChangeAndEvent(d[0].mutation));
            for (const line of expected.lineData) {
                const mutationKey = generateMutationIdByGeneAndProteinChangeAndEvent(line[0].mutation);
                roughlyDeepEqualPoints(mutationKeyToLineData[mutationKey], line, `${messagePrefix||""}mutation with key ${mutationKey}`);
            }
        }
                ...
                ),
                {
                    ...
                    lineData:[
                        ...
                        [
                            {
                                sampleId:"sample1",
                                mutation: makeMutation(1, "gene2", "proteinchange2", 30, "uncalled"),
