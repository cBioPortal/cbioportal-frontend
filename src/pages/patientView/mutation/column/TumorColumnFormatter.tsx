...
export default class TumorColumnFormatter implements IColumnFormatter {
    ...
    static numCmp = (a,b) => (a > b ? 1 : (a === b ? 0 : -1));
    ...
    public static renderFunction(data:IColumnFormatterData, columnProps:any) {
        const sampleIndex = columnProps.sampleOrder.reduce((map, sampleId, i) => {map[sampleId] = i; return map;}, {});
        ...
        const circles = samples.map(function(sampleId) {
            return (
                <Tooltip placement="right" overlay={tooltipText(sampleId)} arrowContent={<div className="rc-tooltip-arrow-inner"></div>}>
                    <svg width={2*TumorColumnFormatter.circleRadius} height={2*TumorColumnFormatter.circleRadius} style={{'position':'absolute', 'left':svgX[sampleId]}}>
                        <g key={sampleId} transform={`translate(${TumorColumnFormatter.circleRadius},${TumorColumnFormatter.circleRadius})`}>
                            <circle r={TumorColumnFormatter.circleRadius} fill={columnProps.sampleColors[sampleId]}></circle>
        ...
        return (
            <Td column={data.name} value={columnProps.sampleOrder.map(sampleId => (presentSamples[sampleId] ? 1 : 0))}>
