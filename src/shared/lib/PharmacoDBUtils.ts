...
import {IPharmacoDBGeneDrugAssociationData, IPharmacoDBView,IPharmacoDBViewList, IPharmacoDBCnaRequest} from "shared/model/PharmacoDB.ts";
 ...
 */
export function getPharmacoDBCnaView(oncoTreeCode: string, cnareqlist: Array<IPharmacoDBCnaRequest>): Promise<IPharmacoDBViewList> {
    ...
    cnareqlist.forEach(function(cnareq){
        ...
        if(genes.length > 100)
        {
            promises.push(pharmacoDBClient.getCNAView(oncoTreeCode,genes.join(),statuses.join()));
            genes = [];
            statuses = [];
        }
    });
