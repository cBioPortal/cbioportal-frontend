...
const maxCacheSize = megabytes(450); // set to Number.POSITIVE_INFINITY to disable cache clearing
let cacheSize = 0;
...
function tryFreeCache() {
    ...
    try {
        while (cacheSize > maxCacheSize) {
            ...
            //console.log(`POST cache max size exceeded: ${cacheSize} > ${maxCacheSize}. Deleting entries.`);
