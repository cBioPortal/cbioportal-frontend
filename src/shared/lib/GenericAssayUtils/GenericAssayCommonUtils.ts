...
export async function fetchGenericAssayMetaByMolecularProfileIdsGroupByGenericAssayType(
    ...
    await Promise.all(
        genericAssayTypes.map(async genericAssayType => {
            const genericAssayProfiles =
                genericAssayProfilesGroupByGenericAssayType[genericAssayType];
            const genericAssayProfileIds = genericAssayProfiles.map(
...
export async function fetchGenericAssayData(
    ...
    const params = Object.keys(entityIdsByProfile).map(profileId => ({
