...
export function perform_search_single<T>(parsed_query:SearchClause[], item:T, matcher:(phrase:string, item:T)=>boolean):SearchResult;
export function perform_search_single<T>(parsed_query:SearchClause[], item:T, matcher?:(phrase:string, item:T)=>boolean)
{
	if (!matcher)
		matcher = matchPhrase as any;
