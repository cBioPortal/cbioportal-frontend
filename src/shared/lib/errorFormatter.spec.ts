...
describe("ErrorFormatter", () => {
    describe("formatErrorLog", ()=>{
        it ("formatErrorLog one error", ()=>{
            assert.deepEqual(formatErrorLog(siteErrors.slice(0, 1)), '{"body":{"message":"RESPONSE_BODY_MESSAGE"}}');
            assert.deepEqual(formatErrorLog(siteErrors.slice(1, 2)), REQUEST_ERROR_MESSAGE);
            assert.deepEqual(formatErrorLog(siteErrors.slice(-1)), siteErrors[2].errorObj.toString());
        });
        it ("formatErrorLog multiple errors", ()=>{
            assert.deepEqual(formatErrorLog(siteErrors), '{"body":{"message":"RESPONSE_BODY_MESSAGE"}}\n\n\nREQUEST_ERROR_MESSAGE\n\n\n[object Object]');
        });
    });
    ...
    describe("formatErrorMessages", ()=>{
        ...
        it ("formatErrorMessages multiple errors", ()=>{
            assert.deepEqual(formatErrorMessages(siteErrors), [RESPONSE_BODY_MESSAGE, REQUEST_ERROR_MESSAGE]);
        });
    });
});