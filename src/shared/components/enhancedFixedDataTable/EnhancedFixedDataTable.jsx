...
var EnhancedFixedDataTable = (function() {
    ...
    var ColumnHider = React.createClass({
        ...
        componentDidMount: function() {
            ...
            $('#hide_column_checklist input[type="text"].search').attr('title', 'Search');
        },
    ...
    var PinColumns = React.createClass({
        ...
        componentDidMount: function() {
            ...
            $('#pin_column_checklist input[type="text"].search').attr('title', 'Search');
        },
        ...
        render: function() {
            ...
            var flag =
                data[rowIndex][field] && filterAll.length > 0
                    ? data[rowIndex][field].toLowerCase().indexOf(filterAll.toLowerCase()) >= 0
                    : false;
    ...
    var Main = React.createClass({
                    ...
                    if (_label) {
                        ...
                        switch (measureMethod) {
                            ...
                            default:
                                ...
                                _labelWidth =
                                    upperCaseLength * 10 + (dataLength - upperCaseLength) * 8 + 15;
                                break;
                        }
                        if (_labelWidth > columnWidth[attr]) {
                            var end =
                                Math.floor((_label.length * columnWidth[attr]) / _labelWidth) - 3;
            ...
            _.each(cols, function(col) {
                ...
                if (_label) {
                    ...
                    if (_labelWidth > columnWidth[col.name]) {
                        var end =
                            Math.floor((_label.length * columnWidth[col.name]) / _labelWidth) - 3;
                    ...
                    if (!filters[col].hide) {
                        if (filters[col].type == 'STRING') {
                            if (!row[col] && hasGroupHeader) {
                                ...
                                if (
                                    hasGroupHeader &&
                                    row[col].toLowerCase().indexOf(filters[col].key.toLowerCase()) <
                                        0
                                ) {
                                    return false;
                                }
        ...
        getInitialState: function() {
            var cols = [],
                ...
                newCol,
                measureMethod =
                    dataLength > 100000 || !this.props.autoColumnWidth ? 'charNum' : 'jquery',
        ...
        render: function() {
            var sortDirArrow =
                this.state.sortDir === this.SortTypes.DESC ? 'fa fa-sort-desc' : 'fa fa-sort-asc';
