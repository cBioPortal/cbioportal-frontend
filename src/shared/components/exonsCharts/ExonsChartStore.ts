...
export class ExonsChartStore {
    ...
    readonly transcriptsById = remoteData({
        ...
        invoke: () => {
            const withExons = (this.transcripts.result || []).map(
                ExonsChartStore.addExonProps
            );
    ...
    get fusions(): StructuralVariantExt[] {
        return (this.fusionMapperDataStore || { tableData: [] }).tableData.map(
            (f: StructuralVariantExt[]) => f[0]
        );
    }
    ...
    readonly pfamDomainsById = remoteData({
        ...
        invoke: () =>
            Promise.resolve(
                _.groupBy(this.pfamDomains.result!, p => p.pfamAccession)
            ),
    });
    ...
    getExonsBySite(
        ...
        if (siteId === 1 || siteId === 2) {
            ...
            if (result !== undefined && result[transcriptId] !== undefined) {
                const sortedExons = <ExonRangeExt[]>result[
                    transcriptId
                ][0].exons.sort(
                    // sort exons by rank
                    (exon1: ExonRangeExt, exon2: ExonRangeExt) =>
                        exon1.rank - exon2.rank
                );
    ...
    static updateDeltaX(
        ...
        const isRightAlignedAndLongest =
            !isLeftAligned && refTotalWidth > tTotalWidth;
        const newDeltaX = isRightAlignedAndLongest
            ? tDeltaX - (refTotalWidth - tTotalWidth)
            : tDeltaX;
    ...
    static getSite1LongestFusion(
        previousFusions: StructuralVariantExt[]
    ): StructuralVariantExt | undefined {
        const maxTotalWidth = Math.max.apply(
            Math,
            previousFusions
    ...
    static hasLongestSite1(
        fusion: StructuralVariantExt,
        t: EnsemblTranscriptExt,
        prevLongest: StructuralVariantExt | undefined
    ): boolean {
        ...
        if (prevLongest) {
            ...
            longerThanPreviousLongest = prevLongest
                ? fusionTotalWidth > prevLongest.totalWidth
                : true;
        }
        // Only when it's right aligned
        return (
            fusionTotalWidth > referenceTotalWidth &&
            !fusion.isLeftAligned &&
            longerThanPreviousLongest
        );
    }
    ...
    static addDeltaX(
        ...
        const prevSite1Longest = ExonsChartStore.getSite1LongestFusion(
            prevFusions
        );
        const isFusionHasLongestSite1 = ExonsChartStore.hasLongestSite1(
            newFusion,
            transcript,
            prevSite1Longest
        );
        ...
        if (isFusionHasLongestSite1) {
            ...
            if (prevSite1Longest) {
                prevSite1LongestVal = prevSite1Longest.totalWidth
                    ? prevSite1Longest.totalWidth
                    : prevSite1LongestVal;
            }
            if (
                !newFusion.isLeftAligned &&
                newFusion.totalWidth > refTotalWidth &&
                prevSite1LongestVal > 0
            ) {
    ...
    addExtraTranscriptProps(
        transcript: EnsemblTranscriptExt
    ): { fusions: StructuralVariantExt[]; transcript: EnsemblTranscriptExt } {
        const prevFusions: StructuralVariantExt[] = [];
        const newTranscript = Object.assign(
            { totalWidth: 0, deltaX: 0 },
            transcript
        );
        const newFusions = this.fusions
            ...
            .filter(fusion => {
                ...
                return (
                    fusion.site1EnsemblTranscriptId ===
                        newTranscript.transcriptId ||
                    fusion.site2EnsemblTranscriptId ===
                        newTranscript.transcriptId
                );
            })
            .map(fusion =>
                ExonsChartStore.addDeltaX(fusion, newTranscript, prevFusions)
            )
            .map(fusion =>
                ExonsChartStore.updateDeltaX(
                    fusion,
    ...
    static addExonRangeProps(
        exonRanges: ExonRangeExt[],
    ...
    static addExonProps(
        ...
        const totalWidth = ExonsChartStore.getTotalWidth(<ExonRangeExt[]>(
            newExons
        ));
    ...
    static getFivePrimeLength(transcript: EnsemblTranscriptExt): number {
        return !transcript.utrs
            ? 0
            : transcript.utrs.reduce((accumulator, utr) => {
                  // only calculate five prime
                  let fivePrimeWidth =
                      utr.type === UtrType.FivePrime ? utr.end - utr.start : 0;
    ...
    getPfamDomainDetails(
        pfamDomainRangeExts: PfamDomainRangeExt[]
    ): PfamDomainRangeExt[] {
        return (
            (pfamDomainRangeExts || [])
                // do not include empty objects
                .filter(
                    (pfam: PfamDomainRangeExt) => Object.keys(pfam).length > 0
                )
