...
> {
    @computed get message() {
        ...
        if (!this.props.oql.query.length) return null;
        ...
        if (
            !this.props.hideValidatingMessage &&
            this.props.genes.isPending &&
            this.props.genes.result!.suggestions.length === 0
        )
            return (
                <div className={styles.GeneSymbolValidator}>
                    <span className={styles.pendingMessage}>
        ...
        if (this.props.genes.result!.suggestions.length)
            return (
                <div className={styles.GeneSymbolValidator}>
                    ...
                    {this.props.genes.result!.suggestions.map(
        ...
        if (this.props.hideSuccessMessage) return null;
    ...
    render() {
        return <div id="geneBoxValidationStatus">{this.message}</div>;
    }
        ...
        return (
            <div key={key} className={styles.suggestionBubble} title={title}>
                ...
                <ReactSelect
                    ...
                    options={options}
                    onChange={(option: any) =>
                        option && this.replaceGene(alias, option.value)
                    }
                    autosize
                />
            </div>
