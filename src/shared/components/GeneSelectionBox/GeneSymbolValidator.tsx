...
export interface IGeneSymbolValidatorProps {
	...
	afterValidation?: (validQuery: boolean, validationResult: GeneValidationResult, oql:OQL) => void;
	wrap?: boolean;
}

export type GeneValidationResult = { found: Gene[], suggestions: GeneReplacement[] };
...
export default class GeneSymbolValidator extends React.Component<IGeneSymbolValidatorProps, {}> {
	...
	readonly genes = remoteData({
		invoke: async (): Promise<{ found: Gene[], suggestions: GeneReplacement[] }> => {
		...
		onResult: (genes) => {
			if (this.props.afterValidation) {
				this.props.afterValidation(genes.suggestions.length === 0, this.genes.result, this.oql);
			}
		},
