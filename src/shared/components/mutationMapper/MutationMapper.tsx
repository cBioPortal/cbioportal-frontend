...
> extends React.Component<P, {}> {
    ...
    @computed get trackDataStatus(): TrackDataStatus {
        ...
        if (
            ...
            _.isEmpty(this.props.store.hotspotsByProteinPosStart)
        ) {
    ...
    @computed get geneSummary(): JSX.Element {
        ...
        return (
            <div style={{ paddingBottom: 10 }}>
                ...
                <Observer>{this.renderDropdown}</Observer>
                <div>
                ...
                {showDropDown
                    ? store.activeTranscript && (
                          <div>
                              <span>Ensembl: </span>
                              <a
                                  href={`http://grch37.ensembl.org/homo_sapiens/Transcript/Summary?t=${
                                      store.activeTranscript
                                  }`}
                                  target="_blank"
                              >
    ...
    private renderDropdown() {
        ...
        if (!showDropDown) {
            ...
            const isLoading =
                store.transcriptsWithProteinLength.isPending ||
                store.transcriptsWithAnnotations.isPending ||
                store.canonicalTranscript.isPending;
            const requiredData =
                store.indexedVariantAnnotations.result &&
                Object.keys(store.indexedVariantAnnotations.result).length >
                    0 &&
                canonicalTranscriptId &&
            ...
            [
                ...
                t =>
                    this.props.store.transcriptsByTranscriptId[
                        t
                    ].hasOwnProperty('refseqMrnaId'),
                t =>
                    this.props.store.transcriptsByTranscriptId[t].proteinLength,
                t => t,
            ],
            ['desc', 'desc', 'desc', 'asc']
        );
    ...
    protected onTrackVisibilityChange(selectedTrackNames: string[]) {
        ...
        if (
            this.trackVisibility[TrackNames.PDB] === 'hidden' &&
            selectedTrackNames.includes(TrackNames.PDB)
        ) {
            this.open3dPanel();
        }
