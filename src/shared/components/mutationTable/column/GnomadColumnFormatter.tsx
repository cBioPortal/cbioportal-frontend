...
export default class GnomadColumnFormatter {
    ...
    private static getGnomadDataViz(genomeNexusCacheData:GenomeNexusCacheDataType|null) {
        ...
        if (genomeNexusCacheData == null) {
            ...
            if (gnomadData && (gnomadData.gnomadExome || gnomadData.gnomadGenome)) {
                ...
                if (gnomadData.gnomadExome && gnomadData.gnomadGenome) {
                    Object.keys(PopulationName).map(key => {
                        gnomadResult[key] = {
                            ...
                            'alleleFrequency': parseFloat(((gnomadExome[key].alleleCount + gnomadGenome[key].alleleCount) / 
                                            (gnomadExome[key].alleleNumber + gnomadGenome[key].alleleNumber)).toFixed(10))
                        } as GnomadData;
                    })
                    result = gnomadResult;
                }
                ...
                display = parseFloat(result["Total"].alleleFrequency.toFixed(10)).toString();
    ...
    public static setGnomadTableData (key: string, data: Gnomad, result: {[key:string]: GnomadData}) {
        ...
        const alleleCountName : keyof AlleleCount = (PopulationName[key] ? ColumnName.alleleCount + "_" + PopulationName[key] : ColumnName.alleleCount).toString() as keyof AlleleCount;
        const alleleNumberName : keyof AlleleNumber = (PopulationName[key] ? ColumnName.alleleNumber + "_" + PopulationName[key] : ColumnName.alleleNumber).toString() as keyof AlleleNumber;
        const homozygotesName : keyof Homozygotes = (PopulationName[key] ? ColumnName.homezygotes + "_" + PopulationName[key] : ColumnName.homezygotes).toString() as keyof Homozygotes;
        const alleleFrequencyName : keyof AlleleFrequency = (PopulationName[key] ? ColumnName.alleleFrequency + "_" + PopulationName[key] : ColumnName.alleleFrequency).toString() as keyof AlleleFrequency;
        ...
        result[key] = 
        {
            ...
            'alleleFrequency': data.alleleFrequency[alleleFrequencyName]
        } as GnomadData;
    }
    ...
    public static getSortValue(data:Mutation[], genomeNexusCache:GenomeNexusCache): number|null {
        ...
        if (genomeNexusCacheData && genomeNexusCacheData.data) {
            ...
            if (gnomadData && gnomadData.gnomadExome && gnomadData.gnomadGenome) {
                return parseFloat(((gnomadData.gnomadExome.alleleCount.ac + gnomadData.gnomadGenome.alleleCount.ac) /
                                    (gnomadData.gnomadExome.alleleNumber.an + gnomadData.gnomadGenome.alleleNumber.an)).toFixed(10));               
            }

            // If only has gnomadExome, sort by gnomadExome frequency
            if (gnomadData && gnomadData.gnomadExome) {
                return parseFloat(gnomadData.gnomadExome.alleleFrequency.af.toString());
            }

            // If only has gnomadGenome, sort by gnomadGenome frequency
            if (gnomadData && gnomadData.gnomadGenome) {
                return parseFloat(gnomadData.gnomadGenome.alleleFrequency.af.toString());
            }
        }
    ...
    public static download(data:Mutation[], genomeNexusCache:GenomeNexusCache): string
    {
        ...
        if (genomeNexusCacheData && genomeNexusCacheData.data) {
            ...
            if (gnomadData && gnomadData.gnomadExome && gnomadData.gnomadGenome) {
                return ((gnomadData.gnomadExome.alleleCount.ac + gnomadData.gnomadGenome.alleleCount.ac) /
                        (gnomadData.gnomadExome.alleleNumber.an + gnomadData.gnomadGenome.alleleNumber.an)).toFixed(10);               
            }

            if (gnomadData && gnomadData.gnomadExome) {
                return gnomadData.gnomadExome.alleleFrequency.af.toString();
            }

            if (gnomadData && gnomadData.gnomadGenome) {
                return gnomadData.gnomadGenome.alleleFrequency.af.toString();
            }
        }
