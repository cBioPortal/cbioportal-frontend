...
export default class DbsnpColumnFormatter {
    ...
    public static renderFunction(data: Mutation[],
        ...
        const genomeNexusCacheData = DbsnpColumnFormatter.getGenomeNexusDataFromCache(data, genomeNexusCache);
        return (
                ...
                return (
                    ...
                    >
                        <span style={{textAlign:"right", float:"right"}}>
                            <a href={dbsnpLink} target="_blank">{rsId}</a>
                        </span>
                    </DefaultTooltip>
                );
            }        
    ...
    public static getData(genomeNexusData: MyVariantInfoAnnotation | null): string | null {
        if (genomeNexusData && genomeNexusData.annotation && genomeNexusData.annotation.dbsnp && genomeNexusData.annotation.dbsnp.rsid) {
    ...
    public static download(data: Mutation[], genomeNexusMyVariantInfoCache: GenomeNexusMyVariantInfoCache): string {
        const genomeNexusCacheData = DbsnpColumnFormatter.getGenomeNexusDataFromCache(data, genomeNexusMyVariantInfoCache);
        if (genomeNexusCacheData && genomeNexusCacheData.data) {
            const dbsnpData = genomeNexusCacheData && DbsnpColumnFormatter.getData(genomeNexusCacheData.data.my_variant_info);
            if (dbsnpData) {
                return dbsnpData;
            }
        }
    ...
    public static getSortValue(data: Mutation[], genomeNexusCache: GenomeNexusMyVariantInfoCache): string | null {
        const genomeNexusCacheData = DbsnpColumnFormatter.getGenomeNexusDataFromCache(data, genomeNexusCache);
