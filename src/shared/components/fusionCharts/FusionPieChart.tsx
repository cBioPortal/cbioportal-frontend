...
export interface IFusionPieChartProps {
    store: FusionMapperDataStore;
    fusionCounts: { [fusedGene: string]: number };
}

@observer
export default class FusionPieChart extends React.Component<
    IFusionPieChartProps,
    {}
> {
    @observable geneFilters: string[] = [];
    ...
    toggleSelection(str: string) {
        ...
        if (this.geneFilters.length) {
            // apply filters
            this.props.store.setFilter((d: StructuralVariantExt[]) => {
            ...
            this.props.store.setFilter(d => true);
        }
    }
    ...
    public render() {
        return (
            <VictoryPie
                ...
                labels={(d: any) =>
                    `${
                        d.y / this.props.store.tableData.length > 0.1 ? d.y : ''
                    }`
                }
                events={[
                    {
