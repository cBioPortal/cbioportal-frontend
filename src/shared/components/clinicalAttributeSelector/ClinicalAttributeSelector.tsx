...
@observer
export default class ClinicalAttributeSelector extends React.Component<
    IClinicalAttributeSelectorProps,
    {}
> {
    ...
    readonly sortedClinicalAttributes = remoteData({
        ...
        invoke: () => {
            const availableSampleCount = this.props.store
                .clinicalAttributeIdToAvailableSampleCount.result!;
            return Promise.resolve(
                _.sortBy<ClinicalAttribute>(
                    ...
                    [
                        (x: ClinicalAttribute) => {
                            if (
                                ...
                                x.clinicalAttributeId.startsWith(
                        ...
                        (x: ClinicalAttribute) => {
                            let sampleCount =
                                availableSampleCount[x.clinicalAttributeId];
    ...
    readonly options = remoteData({
        ...
        invoke: () => {
            ...
            const clinicalAttributeIdToAvailableSampleCount = this.props.store
                .clinicalAttributeIdToAvailableSampleCount.result!;
            return Promise.resolve(
                _.uniqBy(
                    this.sortedClinicalAttributes.result!,
                    attr => attr.clinicalAttributeId
                ).reduce(
                    (
                        options: { label: string; value: string }[],
                        next: ExtendedClinicalAttribute
                    ) => {
                        let sampleCount =
                        ...
                        if (
                            ...
                            next.clinicalAttributeId.startsWith(
                                SpecialAttribute.ProfiledInPrefix
                            )
                        ) {
                        ...
                        const newOption = {
                            label: `${next.displayName} (${getPercentage(
                                sampleCount / totalSampleCount,
                                0
                            )})`,
    ...
    @autobind private addAll() {
        onMobxPromise(this.options, options => this.onChange(options));
    }
        ...
        return (
            <span onFocus={this.onFocus}>
                <CheckedSelect
                    ...
                    addAllLabel={`Add all ${
                        this.options.peekStatus === 'complete'
                            ? '(' + this.options.result!.length + ')'
                            : ''
                    }`}
                />
            </span>
