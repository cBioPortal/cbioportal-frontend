...
export const LEGEND_Y =
    CORRELATION_INFO_Y +
    90; /* 90 ≈ approximate correlation info height + top padding */
...
> extends React.Component<IScatterPlotProps<D>, {}> {
    ...
    private get regressionLine() {
        if (this.props.showRegressionLine && this.props.data.length >= 2) {
            const regressionLineComputations = getRegressionComputations(
                this.props.data.map(
            ...
            data[1].label = [
                regressionLineComputations.string,
                `R² = ${regressionLineComputations.r2}`,
            ];
            return [
                <VictoryLine
                    style={{
