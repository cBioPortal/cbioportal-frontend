...
> {
    ...
    @computed get horzAxis() {
        ...
        if (this.props.horizontalBars) {
            label.unshift(
                `${COUNT_AXIS_LABEL}${this.props.percentage ? ' (%)' : ''}`
            );
        }
        return (
            <VictoryAxis
                ...
                tickCount={
                    this.props.horizontalBars ? NUM_AXIS_TICKS : undefined
                }
                tickFormat={
                    this.props.horizontalBars
    ...
    @computed get vertAxis() {
        ...
        return (
            <VictoryAxis
                ...
                tickCount={
                    this.props.horizontalBars ? undefined : NUM_AXIS_TICKS
                }
                tickFormat={
                    this.props.horizontalBars
                        ? this.formatCategoryTick
    ...
    private tooltip(datum: any) {
        return (
            <div>
                <strong>{datum.majorCategory}</strong>
                <br />
                <strong>{datum.minorCategory}</strong>:&nbsp;{datum.count}
                &nbsp;sample{datum.count === 1 ? '' : 's'}&nbsp;(
                {datum.percentage}%)
            </div>
        );
    }
        ...
        if (this.data.length > 0) {
            return (
                ...
                >
                    ...
                    >
                        <g
                            transform={`translate(${this.leftPadding}, ${
                                this.topPadding
                            })`}
                        >
                            <VictoryChart
    ...
    private getTooltip() {
        if (this.container && this.tooltipModel) {
            const countAxisOffset =
                (this.tooltipModel.y + this.tooltipModel.y0) / 2;
                ...
                <ScatterPlotTooltip
                    ...
                    targetCoords={{
                        x:
                            (this.categoryAxis === 'x'
                                ? categoryAxisOffset
                                : countAxisOffset) + this.leftPadding,
                        y:
                            (this.categoryAxis === 'y'
                                ? categoryAxisOffset
                                : countAxisOffset) + this.topPadding,
                    }}
