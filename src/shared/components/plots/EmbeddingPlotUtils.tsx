...
import { IPlotSampleData, makeScatterPlotPointAppearance } from './PlotsTabUtils';
import { StudyViewPageStore } from '../../../pages/studyView/StudyViewPageStore';
import { Sample } from 'cbioportal-ts-api-client';
import { aggregateMolecularDataByPatient, createSampleLookupMap, preComputeClinicalDataMaps, getMolecularDataForGeneSync } from '../../lib/PatientMolecularDataUtils';
import {
    MUT_COLOR_MISSENSE,
...
export function makeEmbeddingScatterPlotData(
    ...
    const filteredSamplesByDetailedCancerType = store.filteredSamplesByDetailedCancerType.result;
    if (filteredSamplesByDetailedCancerType) {
        for (const [cancerType, samples] of Object.entries(filteredSamplesByDetailedCancerType)) {
    ...
    if (coloringOption?.info?.entrezGeneId && coloringOption.info.entrezGeneId !== -3) {
    ...
    return embeddingCoordinates.map((coord: EmbeddingCoordinate): EmbeddingPlotPoint => {
        ...
        if (sample && coloringOption) {
            if (coloringOption.info?.entrezGeneId && coloringOption.info.entrezGeneId !== -3) {
                ...
                const molecularData = patientMolecularDataMap.get(coord.patientId) || {
                    mutations: [], cnas: [], svs: [], hasAnyAlteration: false
                };
                ...
                // Use standard cBioPortal colors directly (same as backup file approach)
                if (molecularData.svs.length > 0) {
                    ...
                    const mutationType = firstMutation.mutationType || firstMutation.type || 'unknown';
                    const lowerType = mutationType.toLowerCase();
                    const driversAnnotated = store.driverAnnotationSettings?.driversAnnotated || false;
                    ...
                    if (lowerType.includes('missense')) {
                        cancerType = 'Missense';
                        color = driversAnnotated && !isDriver ? MUT_COLOR_MISSENSE_PASSENGER : MUT_COLOR_MISSENSE;
                    } else if (lowerType.includes('truncating') || lowerType.includes('nonsense') || 
                              lowerType.includes('stop_gained') || lowerType.includes('frame_shift') || 
                              lowerType.includes('frameshift')) {
                        cancerType = 'Truncating';
                        color = driversAnnotated && !isDriver ? MUT_COLOR_TRUNC_PASSENGER : MUT_COLOR_TRUNC;
                    } else if (lowerType.includes('frame_del') || lowerType.includes('frame_ins') || 
                              lowerType.includes('in_frame_del') || lowerType.includes('in_frame_ins') || 
                              lowerType.includes('inframe')) {
                        cancerType = 'Inframe';
                        color = driversAnnotated && !isDriver ? MUT_COLOR_INFRAME_PASSENGER : MUT_COLOR_INFRAME;
                    } else if (lowerType.includes('splice')) {
                        cancerType = 'Splice';
                        color = driversAnnotated && !isDriver ? MUT_COLOR_SPLICE_PASSENGER : MUT_COLOR_SPLICE;
                    } else {
                        cancerType = 'Other';
                        color = driversAnnotated && !isDriver ? MUT_COLOR_OTHER_PASSENGER : MUT_COLOR_OTHER;
                    }
                    
                    // Add driver annotation to label
                    if (driversAnnotated && firstMutation.putativeDriver !== undefined) {
                        cancerType = isDriver ? `${cancerType} (Driver)` : `${cancerType} (VUS)`;
                    }
                    strokeColor = color;
                } else if (molecularData.cnas.length > 0) {
                    const firstCna = molecularData.cnas[0];
                    const cnaValue = firstCna.value !== undefined ? firstCna.value : 0;
                ...
                cancerType = clinicalDataValueMap.get(sampleKey) || 
                    patientToCancerTypeMap.get(coord.patientId) || 
                    'Unknown';
            }
        } else if (sample) {
            // Default coloring
            cancerType = patientToCancerTypeMap.get(coord.patientId) || 'Unknown';
        }
