...
import {
    default as StructureVisualizer, ProteinScheme, IResidueSpec,
    IStructureVisualizerProps, MutationColor, SideChain
...
export default class PyMolScriptGenerator extends StructureVisualizer
{
    ...
    public static get PROTEIN_SCHEME_PRESETS(): {[scheme:number]: string}
    {
        ...
        presets[ProteinScheme.BALL_AND_STICK] = "hide everything; show spheres; show sticks; alter all, vdw=0.50";
    ...
    public generateScript(pdbId: string,
                          chainId: string,
                          residues: IResidueSpec[],
                          props: IStructureVisualizerProps): string
    {
        this._script = [];
    ...
    {
        this._script.push(`select ((resi ${residueCodes.join(",")}) and (chain ${chainId}) and (not name c+n+o));`);
    }
    ...
    {
        const proteins = "asp+glu+arg+lys+his+asn+thr+cys+gln+tyr+ser+gly+ala+leu+val+ile+met+trp+phe+pro";
                                 ...
                                 props: IStructureVisualizerProps)
    {
        ...
        const grouped:{[color:string]: IResidueSpec[]} = _.groupBy(residues, (residue: IResidueSpec) => {
