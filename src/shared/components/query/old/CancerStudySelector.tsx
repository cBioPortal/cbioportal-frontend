...
export default class CancerStudySelector extends React.Component<ICancerStudySelectorProps, ICancerStudySelectorState>
{
	...
	@memoizeWith('getMemoizeParams')
	getTreeDescriptor():ITreeDescriptor<CancerType|CancerStudy> & {rootNode: CancerType, setExpanded: <T>(node:CancerTreeNode, value:boolean, event:React.MouseEvent<T>) => void, nodeFilter: (node:CancerTreeNode) => boolean}
	{
		let {
			rootCancerType,
			map_node_meta,
	...
	@memoize
	getTreeSelectHandler<T>(node:CancerTreeNode)
	{
	...
	@memoize
	getOnCheckHandler<T>(node:CancerTreeNode)
	{
	...
	getListItemProps = (node:CancerTreeNode):React.HTMLAttributes<HTMLLIElement> =>
	{
