...
export default class QueryContainer extends React.Component<QueryContainerProps, {}> {
    ...
    @computed get studiesDataReady(): boolean {
        return this.store.studiesHaveChangedSinceInitialization ||
    ...
    @computed get selectedStudySummary(){
        ...
        if (this.store.selectableSelectedStudies.length > LIMIT) {
            const overlay = ()=>this.store.selectableSelectedStudies.slice(LIMIT).map((study)=><div>{study.name}</div>);
            overflow = <span className={'inlineBlock'}><DefaultTooltip overlay={overlay}><a>and {this.store.selectableSelectedStudies.length - LIMIT} more</a></DefaultTooltip></span>;
        }

        return <span className={'inlineBlock'}>
            { this.store.selectableSelectedStudies.slice(0,LIMIT).reduce(
                (acc:(string|JSX.Element)[], study, i, arr)=>{
            ...
            {overflow}
            { overflow && (<span>&nbsp;</span>)}
            (<b>{this.store.profiledSamplesCount.result.all}</b> {this.store.profiledSamplesCount.result.all === 1 ? 'sample' : 'total samples'})
                        ...
                        <Then>
                            <If condition={this.props.forkedMode && this.showQueryControls}>
                                    <FlexRow className={styles.StudySelection}>
                                        <SectionHeader className={'sectionLabel'}>
                                            Selected Studies: <button data-test="modifyStudySelectionButton" className={'btn btn-default btn-xs'} onClick={this.toggleQueryControls}>Modify</button>
                                        </SectionHeader>
                            ...
                            <If condition={this.showQueryControls}>
                                <>
                                    ...
                                    <FlexRow overflow={true} padded
                                        ...
                                        <button style={{paddingLeft: 50, paddingRight: 50, marginRight: 50}}
                                                disabled={!this.store.submitEnabled} className="btn btn-primary btn-lg"
                            ...
                            <If condition={!this.showQueryControls}>
                                <FlexRow className={styles.forkedButtons}>
                                    <div style={{width:330,textAlign:'right', marginRight:20}}>
                                        <StudySelectorStats store={this.store}/>
                                    </div>

                                    <DefaultTooltip placement={"top"}
                                                    trigger={["hover"]}
                                        ...
                                        <a data-test="queryByGeneButton"
                                           onClick={()=>this.store.hasSelectedStudies && this.toggleQueryControls()}
                                           data-event={serializeEvent({action:'clickQueryByGeneButton',label:'', category:'homePage' })}
                                                className={classNames('btn btn-primary btn-lg', { disabled: !this.store.hasSelectedStudies } )}>
                                            Query By Gene
                                        </a>
                                    </DefaultTooltip>
                                    OR
                                    <DefaultTooltip placement={"top"}
                                            ...
                                            <a onClick={()=>!this.exploreCohortsButtonDisabled && this.store.openSummary()}
                                                data-event={serializeEvent({action:'clickExploreStudiesButton',label:'', category:'homePage' })}
                                                className={classNames('btn btn-primary btn-lg', { disabled: this.exploreCohortsButtonDisabled } )}>
                                                    <i style={{marginTop:5}} className='ci ci-pie-chart'></i> Explore Selected Studies
                                            </a>
                                    </DefaultTooltip>
                                </FlexRow>
                            </If>
