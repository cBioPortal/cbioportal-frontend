...
**Do not run `yarn run buildAll` to verify a single-file change** — it runs `clean` + rebuilds the vendor DLL + rebuilds all workspace packages (~7 min cold). Use one of these instead:

**Preferred — dev server with HMR (near-instant):**
```bash
yarn start           # or yarn startSSL for HTTPS
```
