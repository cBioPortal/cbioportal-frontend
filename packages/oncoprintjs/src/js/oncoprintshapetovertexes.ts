...
function extractRGBA(color:string | [number,number,number,number]):[number,number,number,number] {
    ...
    const match = color.match(/^[\s]*rgba\([\s]*([0-9]+)[\s]*,[\s]*([0-9]+)[\s]*,[\s]*([0-9]+)[\s]*,[\s]*([0-9.]+)[\s]*\)[\s]*$/);
...
function rectangleToVertexes(params:ComputedShapeParams, z_index:number, addVertex:AddVertexCallback) {
    const x = params.x || 0, y = params.y || 0, height = params.height || 0, width = params.width || 0;

    // Fill
    const fill_rgba = extractRGBA(params.fill as any);
    ...
    // Stroke
    const stroke_width = params['stroke-width'] || 0;
    if (stroke_width > 0) {
        // left side
        const stroke_rgba = extractRGBA(params.stroke as any);
...
function triangleToVertexes(params:ComputedShapeParams, z_index:number, addVertex:AddVertexCallback) {
    const fill_rgba = extractRGBA(params.fill as any);
    addVertex([params.x1 || 0, params.y1 || 0, z_index], fill_rgba);
    addVertex([params.x2 || 0, params.y2 || 0, z_index], fill_rgba);
    addVertex([params.x3 || 0, params.y3 || 0, z_index], fill_rgba);
}

function ellipseToVertexes(params:ComputedShapeParams, z_index:number, addVertex:AddVertexCallback) {
    const center = {x:(params.x || 0) + (params.width || 0) / 2, y: (params.y || 0) + (params.height || 0) / 2};
    const horzrad = (params.width || 0) / 2;
    const vertrad = (params.height || 0) / 2;
...
function lineToVertexes(params:ComputedShapeParams, z_index:number, addVertex:AddVertexCallback) {
    ...
    const half_stroke_width = (params['stroke-width'] || 0) / 2;
