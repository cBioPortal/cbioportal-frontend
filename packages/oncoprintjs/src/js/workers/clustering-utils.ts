 ...
 */
export function hclusterCases(casesAndEntitites:CasesAndEntities):CaseItem[] {
 ...
 */
export function hclusterGeneticEntities(casesAndEntitites:CasesAndEntities):EntityItem[] {
    var refEntityList = null;
    var inputItems = [];
    var refCaseIdList = [];
    //add orderedValueList to all items, so the values are
    ...
    for (var i = 0; i < (refEntityList?.length || 0); i++) {
